...
class Result<T> with _$Result<T> {
  ...
  static Result<T> guard<T>(T Function() body) {
    try {
      ...
      return Result.failure(error: AppError(error: e));
    }
  }
  ...
  static Stream<Result<T>> guardStream<T>(Stream<T> Function() stream) {
    return stream()
        .map(
...
extension ResultObjectExt<T> on T {
  ...
  Result<T> asFailure(Exception e) => Result.failure(error: AppError(error: e));
}
